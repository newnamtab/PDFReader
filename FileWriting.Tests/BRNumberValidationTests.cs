...
namespace FileWriting.Tests
{
    public class BRNumberValidationTests
    {
        ...
        {
            var sut = GetSut(@"\bBR\d{5}\b");
            var result = sut.IsValid(brNumber);
            Assert.Equal(expected, result);
        }
        private BRNumberValidation GetSut(string settingsBRNumberRegexForValidation) {

            return new BRNumberValidation(Options.Create(new FileWriterSettings
            {
                BRNumberRegex = settingsBRNumberRegexForValidation
            }) );
        }
    }
}
